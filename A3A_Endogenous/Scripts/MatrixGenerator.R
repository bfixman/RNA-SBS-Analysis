...
gtf_txdb <- makeTxDbFromGFF("/Users/benfixman/Library/Mobile Documents/com~apple~CloudDocs/Chen_Lab/RNA_SBS/APOBEC3A cytidine deaminase induces RNA editing in monocytes and macrophages/Data/reference files/gencode.v19.annotation.gtf_withproteinids", format = "gtf")

# Check and fix sequence names
gtf_file <- "/Users/benfixman/Library/Mobile Documents/com~apple~CloudDocs/Chen_Lab/RNA_SBS/APOBEC3A cytidine deaminase induces RNA editing in monocytes and macrophages/Data/reference files/gencode.v19.annotation.gtf_withproteinids"
txdb <- makeTxDbFromGFF(gtf_file, format = "gtf")

# Fix sequence names
seqlevelsStyle(txdb) <- "UCSC" # Ensure UCSC naming style for the GTF file
seqlevelsStyle(BSgenome.Hsapiens.UCSC.hg19) <- "UCSC" # Ensure UCSC naming style for the reference genome

genes_hg19 <- genes(txdb)

genes_hg19 <- genes(gtf_txdb)

rna_editing_table <- read.csv("/Users/benfixman/Library/Mobile Documents/com~apple~CloudDocs/Chen_Lab/RNA_SBS/APOBEC3A cytidine deaminase induces RNA editing in monocytes and macrophages/Data/variants/polarization-variants_updated.csv")
...
rna_editing_table$start_seq <-  as.character(getSeq(BSgenome.Hsapiens.UCSC.hg19, flanks_start))
rna_editing_table$end_seq <-  as.character(getSeq(BSgenome.Hsapiens.UCSC.hg19, flanks_end))
...
overlap_genes <- mergeByOverlaps(genes_hg19, editing_pos)
...
strand_varpos <- data.frame(chr = as.character(seqnames(overlap_genes$editing_pos)),
                            ...
                            strand = as.character(strand(overlap_genes$genes_hg19)),
                            stringsAsFactors = FALSE)
...
write.table(trinuc_mat, "/Users/benfixman/Library/Mobile Documents/com~apple~CloudDocs/Chen_Lab/RNA_SBS/APOBEC3A cytidine deaminase induces RNA editing in monocytes and macrophages/Data/Matrices/polarization_matrix_updated.tsv", quote = FALSE, sep = "\t")
